...
// Port of ReportService.vb's GerarPdf/GerarExcel (base inventory report) — same 9
// columns, same order, same "GBS Inventory Report" title.
public static class InventoryReportGenerator
{
    ...
    private static string[] RowValues(Equipamento eq) =>
    [
        eq.InternalUid,
        eq.Manufacturer ?? "",
        eq.Model ?? "",
        ...
        eq.CpuModel ?? "",
        eq.RamGb?.ToString() ?? "",
        eq.StorageGb?.ToString() ?? "",
        eq.ConditionStatus ?? "",
        eq.Notes ?? ""
    ];

    public static byte[] GeneratePdf(List<Equipamento> items)
    {
        ...
        doc.Add(new Paragraph("GBS Inventory Report").SetFont(fontBold).SetFontSize(13).SetFontColor(ColorDark).SetMarginBottom(2));
    ...
    public static byte[] GenerateExcel(List<Equipamento> items)
    {
        using var stream = new MemoryStream();
        using (var wb = new XLWorkbook())
        {
            ...
            ws.Cell(1, 4).Value = "GBS Inventory Report";
            ...
            for (var c = 0; c < Columns.Length; c++)
            {
                ws.Column(c + 1).Width = c == Columns.Length - 1 ? 40 : 18;
            }
