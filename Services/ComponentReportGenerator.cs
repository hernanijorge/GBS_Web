...
// Port of ReportService.vb's GerarPdfComponents/GerarExcelComponents — same 12
// columns, same order, same "GBS Components Report" title.
public static class ComponentReportGenerator
{
    ...
    private static string[] RowValues(Component c) =>
    [
        c.ComponentType,
        c.CapacityGb > 0 ? c.CapacityGb.ToString() : "",
        c.SpeedMhz?.ToString() ?? "",
        c.Generation ?? "",
        c.Cpu ?? "",
        c.StorageGb?.ToString() ?? "",
        c.Brand ?? "",
        c.PartNumber ?? "",
        c.ConditionStatus,
        c.Status,
    ...
    public static byte[] GeneratePdf(List<Component> items)
    {
        ...
        doc.Add(new Paragraph("GBS Components Report").SetFont(fontBold).SetFontSize(13).SetFontColor(ColorDark).SetMarginBottom(2));
        doc.Add(new Paragraph($"Generated: {DateTime.Now:yyyy-MM-dd HH:mm:ss}  |  Total items: {items.Count}")
    ...
    public static byte[] GenerateExcel(List<Component> items)
    {
        using var stream = new MemoryStream();
        using (var wb = new XLWorkbook())
        {
            var ws = wb.Worksheets.Add("Components Report");
            const int headerRow = 5;

            ws.Cell(1, 4).Value = "GBS Components Report";
            ws.Cell(1, 4).Style.Font.Bold = true;
            ws.Cell(1, 4).Style.Font.FontSize = 16;
            ws.Range(1, 4, 1, 8).Merge();

            ws.Cell(2, 4).Value = $"Generated: {DateTime.Now:yyyy-MM-dd HH:mm:ss} | Total items: {items.Count}";
            ...
            for (var c = 0; c < Columns.Length; c++)
            {
                ws.Column(c + 1).Width = c == Columns.Length - 1 ? 40 : 16;
            }
